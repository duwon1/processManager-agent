...
async def self_update(agent_dir: str) -> tuple[bool, str]:
    ...
    cmds = " && ".join([
        ...
        f"PIP_NO_CACHE_DIR=1 PIP_DISABLE_PIP_VERSION_CHECK=1 {safe_agent_dir}/.venv/bin/python -m pip install --no-cache-dir --disable-pip-version-check -r {safe_agent_dir}/requirements.txt -q",
    ])
